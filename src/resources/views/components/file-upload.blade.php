<div class="govuk-form-group @error($field) govuk-form-group--error @enderror">
    ...
    <input class="govuk-file-upload @error($field) govuk-file-upload--error @enderror" id="{{ $field }}" name="{{ $field }}" type="file"
           aria-describedby="{{ $field }}-hint @error($field) {{ $field }}-error @enderror" value="{{ old($field, session($field)) }}">
</div>
